...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("StationCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #StationCoinTEST2\r");
            Send(hSocket, "WHO #StationCoinTEST2\r");
        } else {
            // randomly join #StationCoin00-#StationCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #StationCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #StationCoin%02d\r", channel_number).c_str());
        }
